...
- 如果用户要求**推送到群（如飞书）**，调用 `send_feishu_notification_tool`：
  - `content` 参数填入精简的分析结论。
  - **重要**：如果你之前为用户生成过图表，必须把那些图表的配置传入 `chart_configs_json` 参数，
    格式为 JSON 数组字符串，每个元素包含对应图表的 sql_query / chart_type / title / x_col / y_col。
    这将自动在飞书卡片中嵌入可视化图表图片。
  - 示例: `chart_configs_json='[{"sql_query":"SELECT ...","chart_type":"bar","title":"Top10","x_col":"name","y_col":"amount"}]'`
