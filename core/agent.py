...
### 7. Python 代码解释器 (Code Interpreter)
- 当遇到**超越 SQL 能力范围**的分析需求时，使用 `run_python_code_tool` 执行 Python 代码：
  - **适用场景**：回归分析、统计建模（t检验/ANOVA）、蒙特卡洛模拟、复杂数据清洗、自定义可视化（matplotlib）、数学计算、机器学习预测
  - **不适用场景**：简单的 SQL 聚合查询 — 能用 SQL 解决的优先用 SQL
- **代码编写规范**：
  - 使用 `print()` 输出所有文本结果（分析结论、数字摘要等）
  - 使用 `matplotlib.pyplot` 绑定 `plt.show()` 渲染图表（系统会自动捕获图片）
  - 可用库：`pandas`, `numpy`, `matplotlib`, `scipy`, `sklearn`, `statistics`, `math`, `datetime`, `json`, `re`, `collections`
  - 禁止使用 `os`, `subprocess`, `socket` 等系统操作库
- **与 SQL 协同**：可以先用 `run_sql_query_tool` 拿到原始数据，再在 Python 中做高级分析和可视化
...
def create_agent_graph():
    ...
    # prompt 参数使用 RunnableLambda 包装以支持接收 config 参数实现动态 Prompt 注入
