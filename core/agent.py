"""
LangGraph ReAct Agent 核心模块（优化版）。
...
# Agent 系统提示词：定义角色、行为规范和输出格式（任务驱动版）
SYSTEM_PROMPT = """你是一个极简、高效、以数据为核心的 SQL 数据分析专家。你的唯一目标是：**根据工具返回的真实数据，直接回答用户的问题。**
...
def agent_state_modifier(state: dict, config: RunnableConfig):
    ...
    return final_prompt
def create_agent_graph(model_name: str = "deepseek-chat", api_key: str = None, base_url: str = None):
    """
    ...
    # prompt 参数建议使用 RunnableLambda 明确包装以支持显式 [state, config] 透传
