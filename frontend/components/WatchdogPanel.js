...
import { EmptyState, ListSkeleton, LoadingState } from './status';
    ...
    <>
      <ModalShell
        ...
        title={
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-xl font-bold leading-tight text-foreground">数据值班室 (Watchdog)</div>
              <p className="mt-0.5 text-sm text-muted-foreground">自动探测业务异常并主动推送预警</p>
            </div>
          </div>
        }
        headerRight={
          <ToolbarButton variant="primary" onClick={() => setShowAddModal(true)} className="rounded-xl px-4 py-2">
            <PlusIcon size={16} />
            <span>新增监控</span>
          </ToolbarButton>
        }
      >
        {loading ? (
          ...
          <SectionCard
            title="巡检规则"
            description="统一查看、测试和管理当前启用的 Watchdog 任务。"
          >
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              {rules.map(rule => (
                <div key={rule.id} className={cn(ui.panel, 'group p-6 transition-all hover:shadow-md')}>
                  ...
                  <div className="mb-4 line-clamp-2 rounded-xl bg-zinc-950 p-3 font-mono text-[11px] text-emerald-300">
                    {rule.sql}
                  </div>

                  <div className="mt-4 flex items-center justify-between border-t border-zinc-200 py-3">
                    <div className="flex flex-col">
                      <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">判断条件</span>
                      <span className="text-sm font-medium text-foreground">当值 {rule.condition} {rule.threshold}</span>
                    </div>
                    <div className="flex flex-col items-end text-right">
                      <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">上次结果</span>
      ...
      <ModalShell
        ...
        bodyClass="p-8"
        title="配置新监控"
        description="填写名称、SQL、判断条件和调度周期，新增一条可被 Prefect 托管的巡检规则。"
      >
        <div className="space-y-4 text-sm">
          <div>
            <label className="mb-1.5 ml-1 block font-semibold text-foreground">规则名称</label>
            <input
          ...
          <div>
            <label className="mb-1.5 ml-1 block text-xs font-semibold text-foreground">查询 SQL (应返回一个数值)</label>
            <textarea
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="mb-1.5 ml-1 block text-xs font-semibold text-foreground">判定方式</label>
              <select
            ...
            <div>
              <label className="mb-1.5 ml-1 block text-xs font-semibold text-foreground">阈值</label>
              <input
                type="number"
                className={ui.inputMuted}
          ...
          <div>
            <label className="mb-1.5 ml-1 block text-xs font-semibold text-foreground">Cron 调度周期 (5段式)</label>
            <input
        ...
        <div className="mt-6 flex gap-3">
          <ToolbarButton
            ...
            className="flex-1 justify-center rounded-2xl py-3"
          >
            取消
          </ToolbarButton>
          <ToolbarButton
